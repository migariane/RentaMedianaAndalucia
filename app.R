    ...
    div(class = "about-section",
      ...
      div(class = "definition-card",
        tags$ul(
          ...
          tags$li(strong("Poblacion extranjera:"), " porcentaje de residentes sin nacionalidad española.")
        )
      ),
      ...
      div(class = "definition-card",
        p(style = "font-size:0.9em;",
          "Si utilizas esta aplicación en una publicación, por favor cítala como:"),
        p(style = "font-size:0.85em; background:#eaf2f8; padding:12px; border-radius:6px; font-style:italic;",
          "Luque-Fernández MA, Massó Guijarro P, Rivas Gervilla G, Nikšić M, Rivera Izquierdo M, ",
          "Montero Alonso MÁ, Melchor Rodríguez JM. ",
...
server <- function(input, output, session) {
  ...
  output$ts_plot <- renderPlotly({
    ...
    # Formato del eje Y según el indicador
  ...
  # ── Renta vs EV scatter plot (con recta de regresion) ──
  output$renta_ev_scatter <- renderPlotly({
    ...
    p <- plot_ly() %>%
      ...
      layout(
        ...
        yaxis = list(title = "Esperanza de vida (anos)", gridcolor = "#e8e8e8"),
        plot_bgcolor = "rgba(0,0,0,0)",
        paper_bgcolor = "rgba(0,0,0,0)",
  ...
  output$causas_ev_bandas <- renderPlotly({
    ...
    # Forzar orden correcto de bandas de edad (evitar orden alfabético)
